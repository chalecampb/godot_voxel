...
CompilationResult Runtime::compile_preprocessed_graph(
	...
	for (size_t order_index = 0; order_index < order.size(); ++order_index) {
		...
		const uint32_t inputs_count = is_script_graph_node ? node.inputs.size() : type.inputs.size();
		const uint32_t outputs_count = is_script_graph_node ? node.outputs.size() : type.outputs.size();
		...
		for (size_t j = 0; j < inputs_count; ++j) {
			...
			if (node.inputs[j].connections.size() == 0) {
				...
				const bool require_input_buffer_when_constant =
						is_script_graph_node ? true : type.inputs[j].require_input_buffer_when_constant;
	...
	// Assign buffer datas
	{
		...
		if (debug) {
			...
			for (unsigned int order_index = 0; order_index < order.size(); ++order_index) {
				...
				const ProgramGraph::Node &node = graph.get_node(node_id);
				const NodeType &type = type_db.get_type(node.type_id);
				const uint32_t outputs_count =
						node.type_id == VoxelGraphFunction::NODE_SCRIPT_GRAPH ? node.outputs.size() : type.outputs.size();
