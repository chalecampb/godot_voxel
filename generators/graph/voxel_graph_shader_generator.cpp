	...
	for (const uint32_t node_id : order) {
		...
		if (is_script_graph_node) {
			...
			if (script_node.is_null()) {
				...
				result.message = "ScriptGraphNode has no VoxelGraphScriptNode resource assigned.";
				result.node_id = node_id;
				return result;
			}
			const VoxelGraphScriptNode::ShaderSource shader_source = script_node->get_shader_source(node_id);
			if (!shader_source.success) {
				CompilationResult result;
				result.success = false;
				result.message = shader_source.error;
				result.node_id = node_id;
				return result;
			}
			codegen.require_lib_code(shader_source.function_name.c_str(), shader_source.source_code.c_str());
