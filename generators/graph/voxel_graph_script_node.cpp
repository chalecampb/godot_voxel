...
void VoxelGraphScriptNodePort::set_port_name(String name) {
	_name = name;
	emit_changed();
}
...
void VoxelGraphScriptNodePort::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_name", "name"), &Self::set_port_name);
	ClassDB::bind_method(D_METHOD("get_name"), &Self::get_port_name);
...
void VoxelGraphScriptNodeParameter::set_parameter_name(String name) {
	_name = name;
	emit_changed();
}
...
void VoxelGraphScriptNodeParameter::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_name", "name"), &Self::set_parameter_name);
	ClassDB::bind_method(D_METHOD("get_name"), &Self::get_parameter_name);
...
bool VoxelGraphScriptNode::reload_script_contract(String script_path) {
	...
	Variant script_instance_variant = script->call("new");
	Ref<VoxelGraphScriptNode> script_instance = script_instance_variant;
	if (script_instance.is_null()) {
		set_script(Variant());
		_script_path = script_path;
		unregister_child_resources();
		_inputs.clear();
	...
	_script_path = script_path;
	set_script(script);
	copy_contract_from_script_instance(**script_instance);
	increment_revision();
	validate();
	_has_deferred_change_notification = true;
...
Ref<VoxelGraphScriptNodePort> VoxelGraphScriptNode::add_input(String name, int type) {
	Ref<VoxelGraphScriptNodePort> port;
	port.instantiate();
	port->set_port_name(name);
...
Ref<VoxelGraphScriptNodePort> VoxelGraphScriptNode::add_output(String name, int type) {
	Ref<VoxelGraphScriptNodePort> port;
	port.instantiate();
	port->set_port_name(name);
	port->set_port_type(type);
...
void VoxelGraphScriptNode::set_parameter_value(String name, Variant value) {
	set(StringName(name), value);
	bool found = false;
	for (Ref<VoxelGraphScriptNodeParameter> &parameter : _parameters) {
		if (parameter.is_valid() && parameter->get_parameter_name() == name) {
...
Variant VoxelGraphScriptNode::get_parameter_value(String name) const {
...
void VoxelGraphScriptNode::copy_contract_from_script_instance(const VoxelGraphScriptNode &script_instance) {
	...
	unregister_child_resources();
	_shader_path = script_instance.get_shader_path();
	_entry_point = script_instance.get_entry_point();
	copy_ports(_inputs, script_instance.get_input_ports());
	copy_ports(_outputs, script_instance.get_output_ports());
	register_child_resources();
}
...
StdVector<VoxelGraphScriptNode::NamedType> VoxelGraphScriptNode::get_exported_parameters_from_property_list() {
	...
	const TypedArray<Dictionary> properties = script->get_script_property_list();
	const StringName name_key("name");
	const StringName type_key("type");
	const StringName usage_key("usage");
	for (int i = 0; i < properties.size(); ++i) {
		const Dictionary property = properties[i];
		const uint32_t usage = property.get(usage_key, 0);
		if ((usage & PROPERTY_USAGE_SCRIPT_VARIABLE) == 0) {
			continue;
		}
		const String name = property.get(name_key, String());
		const Variant::Type type = static_cast<Variant::Type>(static_cast<int>(property.get(type_key, Variant::NIL)));
