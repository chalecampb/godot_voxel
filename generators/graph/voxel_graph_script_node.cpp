...
Variant VoxelGraphScriptNode::create_default_graph_node_resource() {
	Ref<VoxelGraphScriptNode> res;
	res.instantiate();
	return Variant(res);
}

void VoxelGraphScriptNode::compile_graph_node(CompileContext &ctx) {
...
void VoxelGraphScriptNode::process_graph_node_buffer(Runtime::ProcessBufferContext &ctx) {
	...
	for (uint32_t buffer_index = 0; buffer_index < buffer_size; ++buffer_index) {
		for (uint32_t input_index = 0; input_index < params.input_count; ++input_index) {
			...
			if (port.is_valid()) {
				...
				inputs[port->get_port_name()] = input.is_constant ? input.constant_value : input.data[buffer_index];
			}
		}
...
void VoxelGraphScriptNode::analyze_graph_node_range(Runtime::RangeAnalysisContext &ctx) {
	const ScriptGraphNodeParams &params = ctx.get_params<ScriptGraphNodeParams>();
	for (uint32_t output_index = 0; output_index < params.output_count; ++output_index) {
...
void VoxelGraphScriptNode::generate_graph_node_shader(ShaderGenContext &ctx) {
	Ref<VoxelGraphScriptNode> script_node = ctx.get_param(0);
	if (script_node.is_null()) {
...
void VoxelGraphScriptNode::update_registered_graph_node_layout(
		ProgramGraph &graph,
		uint32_t node_id,
...
bool VoxelGraphScriptNode::reload_script_contract(String script_path) {
	if (script_path.is_empty()) {
		add_error("Script path is empty.");
		return false;
	}
	...
	if (script.is_null()) {
		clear_contract(script_path, String("Could not load script `{0}`.").format(varray(script_path)));
		return false;
	}
	...
	if (script_node_instance.is_null()) {
		clear_contract(script_path, String("Script `{0}` must extend VoxelGraphScriptNode.").format(varray(script_path)));
		return false;
	}
...
void VoxelGraphScriptNode::validate_metadata() {
	if (_entry_point.is_empty()) {
		...
		const StdString entry_point_tmp = zylann::godot::to_std_string(_entry_point);
		if (!is_valid_identifier(std::string(entry_point_tmp.c_str()))) {
			add_error(String("Entry point `{0}` must be a valid script and GLSL identifier.").format(varray(_entry_point)));
		}
	}
	...
	for (const Ref<VoxelGraphScriptNodeParameter> &parameter : _parameters) {
		...
		push_unique_name_error(_errors, "parameter", parameter->get_parameter_name(), port_names);
...
StdVector<VoxelGraphScriptNode::NamedType> VoxelGraphScriptNode::get_exported_parameters_from_property_list() {
	...
	for (const zylann::godot::PropertyInfoWrapper &property : properties) {
		...
		if (!is_supported_parameter_type(type)) {
			add_error(String("Script export `{0}` has unsupported type. Use float, int, or bool.")
...
void VoxelGraphScriptNode::sync_parameters_from_script_contract(const StdVector<NamedType> &parameters) {
...
void VoxelGraphScriptNode::validate_script_contract(bool refresh_metadata_from_script) {
	Ref<Script> script = get_script();
	if (script.is_null()) {
		if (!_script_path.is_empty()) {
			add_error(String("Script `{0}` could not be attached to VoxelGraphScriptNode. Check that it extends VoxelGraphScriptNode and compiles.").format(varray(_script_path)));
		}
		return;
	}
	...
	if (!has_method(entry_point)) {
		add_error(String("Script entry point `{0}` was not found in `{1}`.").format(varray(_entry_point, get_script_path())));
	}

#ifdef TOOLS_ENABLED
	if (Engine::get_singleton()->is_editor_hint() && !script->is_tool()) {
		add_error(String("Script `{0}` must be a tool script to run ScriptGraphNode in the editor.")
	...
	if (refresh_metadata_from_script) {
		sync_parameters_from_script_contract(exports);
	}
	validate_name_matches(to_span(exports), to_span(_parameters), "script export", true);
}
...
bool VoxelGraphScriptNode::validate(bool refresh_metadata_from_script, bool force) {
	if (!force && _validated_revision == _revision) {
		return _valid;
	}
	...
	validate_script_contract(refresh_metadata_from_script);
	if (_errors.size() > 0) {
		return false;
	}
	validate_metadata();
